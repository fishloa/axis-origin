...
- **`src/bin/axis-origin.rs`** (`device`) — the ACAP entrypoint: loads config,
...
- **Host** (macOS/Linux, no `device` feature): compiles + tests the pure lib
  (`convert`, `admin`): `cd axis-origin && cargo test`.
...
CI (`.github/workflows/axis-origin.yml`) builds both-arch `.eap`s on every PR
...
docker run --rm -v "$PWD:/w" -w /w/axis-origin \
  ...
  '
# → axis-origin/target/acap/axis-origin_0_1_0_aarch64.eap
```
...
3. A real LL-HLS player (Safari / hls.js / `ffplay`) plays **live** video via
   `https://<cam>/local/axisorigin/hls/cam/media.m3u8` at low latency.
4. The admin `settingPage` loads at `https://<cam>/local/axisorigin/` and a
...
> the nest prefix may need to be dropped (adjust `src/bin/axis-origin.rs`'s
